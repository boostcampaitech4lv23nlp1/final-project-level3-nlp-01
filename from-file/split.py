...
def get_parser():
    ...
    group.add_argument("--mdl_file", type=str, default='mdl/ksponspeech.zip' ,required=False)
    group.add_argument("--wav_scp", type=str, default='download/bert/wav.scp', required=False)
...
class SplitWavAudioMubin():
    def __init__(self, folder, filename):
        ...
        os.makedirs(folder + '/split', exist_ok=True)
        self.audio = AudioSegment.from_wav(self.folder + '/' + self.filename)

        self.filepath = os.path.join(folder, 'split', filename)
        self.scp_texts = []
        self.count = 0
    ...
    def single_split(self, from_min, to_min, split_filename):
        ...
        # metadata
        path = os.path.join(self.folder, 'split', self.filename[:-4]+str(self.count).rjust(2, '0')+self.filename[-4:])[2:]
