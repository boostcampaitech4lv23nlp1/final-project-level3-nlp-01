...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default='/opt/ml/T5/from_hb/', type=str,
    ...
    parser.add_argument('--file_path', default='/opt/ml/data/summary/book/json/test.json', type=str,
    ...
    if args.file_path:
        ...
        all_recall, all_precision, all_rouge = 0, 0, 0
        for i in tqdm(range(len(data))):
            ...
            output = model.generate(**inputs, num_beams=3, do_sample=True, min_length=10, max_length=args.max_target_length)
            decoded_output = tokenizer.batch_decode(output, skip_special_tokens=True)[0]
            result = nltk.sent_tokenize(decoded_output.strip())[0]

            recall, precision, rouge = rouge_n_sentence_level(result, sample, 2)
            all_recall += recall
            all_precision += precision
            all_rouge += rouge
        recall_score = all_recall / len(data)
        precision_score = all_precision / len(data)
        rouge_score = all_rouge / len(data)

        print('ROUGE-2-R', recall_score)
        print('ROUGE-2-P', precision_score)
        print('ROUGE-2-F', rouge_score)
