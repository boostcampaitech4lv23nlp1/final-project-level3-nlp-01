...
def main_test(data_path, sum_model_path, sum_model):
    start = time.time()

    # 1. STT output 파일 데이터셋 가져오기 (STT 후처리 완료된 파일)
    ## 추후 STT output 형식 바뀔 수도 있으니까 따로 구분함
    ...
    summarized_data = summarizer.summarize() # 문단 별 한 문장의 요약문 반환
    ...
    print('time for postprocessing :', postprocess_time - summary_time)
